...
export class Ex3 extends CanvasInitializator {
    constructor() {
        super(...arguments);
        this.divRange = document.getElementById('divRange');
        this.position_x = 0.0;
        this.position_y = 0.0;
        this.position_z = 0.0;
        this.size_point = 10.0;
    }
    sample() {
        ...
        document.getElementById('position_x').addEventListener('change', (ev) => {
            this.position_x = ev.target.value;
            this.redraw();
        });
        document.getElementById('position_y').addEventListener('change', (ev) => {
            this.position_y = ev.target.value;
            this.redraw();
        });
        document.getElementById('position_z').addEventListener('change', (ev) => {
            this.position_z = ev.target.value;
            this.redraw();
        });
        document.getElementById('size_point').addEventListener('change', (ev) => {
